...
class COLMAPDataset:
    def __init__(self, datadir, split, downsample):
        ...
        for image in tqdm(self.images):
            ...
            # 如果是DTU数据集，则将图像转换为RGBA格式
            if self.is_dtu:
                im = im.convert("RGBA")
                rgbas = torch.tensor(np.array(im), dtype=torch.float32) / 255.0
                rgbs = rgbas[..., :3] * rgbas[..., 3:4] + (1 - rgbas[..., 3:4])  # white bg
            else:
                im = im.convert("RGB")
            ...
            if self.is_dtu:
                self.all_alphas.append(rgbas[..., -1:])  # 保存alpha通道

        self.poses = torch.stack(self.poses)
        self.all_rays = torch.stack(self.all_rays)
        self.all_rgbs = torch.stack(self.all_rgbs)
        if self.is_dtu:
            self.all_alphas = torch.stack(self.all_alphas)  # 堆叠alpha通道
